...
export type UseMcpResult = {
  ...
  resourceTemplates: Ref<ResourceTemplate[]>
   ...
   */
  state: Ref<'discovering' | 'pending_auth' | 'authenticating' | 'connecting' | 'loading' | 'ready' | 'failed'>
   ...
   */
  authUrl: Ref<string | undefined>
  /** Array of internal log messages (useful for debugging) */
  log: Ref<{ level: 'debug' | 'info' | 'warn' | 'error'; message: string; timestamp: number }[]>
  /**
